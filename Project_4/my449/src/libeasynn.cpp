...
void append_expression(program *prog, int expr_id, const char *op_name, const char *op_type, int inputs[], int num_inputs) {
    printf("program %p, expr_id %d, op_name %s, op_type %s, inputs %d (", prog, expr_id, op_name, op_type, num_inputs);
...
int add_op_param_ndarray(program *prog, const char *key, int dim, size_t shape[], double data[]) {
    printf("program %p, key %s, value %p dim %d (", prog, key, data, dim);
    for (int i = 0; i != dim; ++i)
        printf("%zu,", shape[i]);
    printf(")\n");
...
void add_kwargs_ndarray(evaluation *eval, const char *key, int dim, size_t shape[], double data[]) {
    printf("evaluation %p, key %s, value %p dim %d (", eval, key, data, dim);
    for (int i = 0; i != dim; ++i)
        printf("%zu,", shape[i]);
    printf(")\n");
...
int execute(evaluation *eval, int *p_dim, const size_t **p_shape, const double **p_data) {
    printf("evaluation %p, p_dim %p, p_shape %p, p_data %p\n", eval, p_dim, p_shape, p_data);
    int status = eval->execute();
    if (status != 0) {
        return status;
    }
    const tensor& t = eval->get_result_tensor();
    *p_dim = t.shape().size();
    *p_shape = t.get_shape_array();
    *p_data = t.get_data_array();
    return 0;
}